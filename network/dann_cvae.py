...
class DANN_VAE:
    def __init__(self, input_size, latent_size=32, batches=2, path=''):
    ...
    def build(self):
        ...
        batch_label = np.argmax(inputs_batch)
...
def DAVAE_Integration(adata_list, epoch=20, batch_size=256, latent_size=6):
    ...
    net = DANN_VAE(input_size=data.shape[1], latent_size=latent_size, batches=batch_num)
    net.build()
    net.compile()
    net.train(data, batch, loss_weight, batch_size=batch_size, epochs=epoch)
    integrate_latent_layer = net.integrate(orig_data, orig_batch)
    adata.obsm['davae_result'] = integrate_latent_layer
    return adata
